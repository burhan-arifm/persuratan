...
<div class="form-group">
    ...
    <select required id="program_studi" name="program_studi" class="form-control selector" form="pengajuan-surat"
        ...
        <option
            {{ $surat->mahasiswa->jurusan->kode_program_studi == $prodi->kode_program_studi ? 'selected ' : '' }}value="{{ $prodi->kode_program_studi }}">
...
<div class="form-group">
    ...
    <textarea required class="form-control" id="alamat" name="alamat"
        ...
        title="Masukkan alamat lengkap Anda tinggal sekarang"
        data-placement="top">{{ $surat->mahasiswa->alamat }}</textarea>
    <div class="invalid-feedback">
        Wajib diisi.
    </div>
</div>
